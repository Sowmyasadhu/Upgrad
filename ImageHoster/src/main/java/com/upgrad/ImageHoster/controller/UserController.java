...
import com.upgrad.ImageHoster.service.UserServiceImpl;
...
public class UserController {
	 ...
	 *
	 * @param session
	 *            HTTP session to check if the user is logged in
	 *
	...
	public String signUpUser(@RequestParam("username") String username, @RequestParam("password") String password,
		...
		if (user1 != null && user1.getUsername().equals(username)) {
			return "users/signup_error";
		} else if (password.length() < 6) {
			return "users/signup_password";
		}
	...
	public String signUpUser1(@RequestParam("username") String username, @RequestParam("password") String password,
	 ...
	 *
	 * @param session
	 *            HTTP session to check if the user has logged in
	 * @return
	 */
	 ...
	 * @param model
	 *            used to pass data to the view for rendering. In this case, the
	 *            model is used to pass errors back to the sign in view if there are
	 *            errors
	 * @param session
	 *            HTTP session to store the signed in user
	 *
	 ...
	 *
	 * @param session
	 *            HTTP session that stores the current user
	 *
	 ...
	 *
	 * @param session
	 *            the HTTP session that tells us if the user is signed in
	 * @param model
	 *            used to pass data to the view for rendering
	 *
	 ...
	 *
	 * @param description
	 *            the updated description for the user
	 * @param file
	...
	/**
	 * This help function converts an image file to base64 encoding, which makes it
	 * easier for us to store the image data in the datbase
	 *
	 * @param file
	 *            the file that we want to convert to base64 encoding
	 *
	 ...
	 *
	 * @param password
	 *            the plain text String that we want to encrypt
	 *
	 * @return the SAH-256 encrypted version of the password
	 */
	private String hashPassword(String password) {
		String passwordHash = Hashing.sha256().hashString(password).toString();
